...
2. **Validación duplicada a propósito.** La función revalida I4–I8 con mensajes explícitos aunque los CHECK y
...
4. **Tope de 12 cuotas** como `CHECK (installments_count <= 12)` en la tabla y como validación en la
   función. Es una regla de producto que no estaba en el schema (solo `>= 1`).
